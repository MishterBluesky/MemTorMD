    ...
    def norloop(norms,ind):
        def nor2loop(total,ind2):
            ...
            def far(acc_total,number):
                return acc_total,number
            acc_total,number = jax.lax.cond((jnp.linalg.norm(poses[ind]-poses[ind2]) < 30)*(jnp.dot(tail_dirs[ind],tail_dirs[ind2]) >0.3),close,far,acc_total,number)
...
def get_normalsv2(poses,tail_dirs):
    ...
    def norloop(norms,ind):
        ...
        def nor2loop(total,ind2):
            ...
            def far(acc_total,number):
                return acc_total,number
            acc_total,number = jax.lax.cond(jnp.linalg.norm(poses[ind]-poses[ind2]) < 30,close,far,acc_total,number)
            return (acc_total,number),ind2
        total,_ = jax.lax.scan(nor2loop,(jnp.array([0.0,0.0,0.0]),0.0),jnp.arange(tail_dirs.shape[0]))
        direc = -total[0]/total[1]
        #direc = poses[ind]-av_pos
...
def calc_curvature(pgrid,ngrid):
    ...
    def nloop(data,ind):
        ...
        def zero(data):
            return data
        data = jax.lax.cond(jnp.logical_or(jnp.linalg.norm(pgrid[ind]) < 1e-5,jnp.linalg.norm(pgrid[ind]) > 20),zero,nzero,data)
...
# -------------------------------------------------
# Median-based outlier filtering (removes explosions)
# -------------------------------------------------
vals = np.array(vals)
...
# robust explosion threshold
threshold = median_val + 7 * mad

print("Median APL:", median_val)
print("MAD:", mad)
print("Outlier threshold:", threshold)

valid_mask = np.logical_and(vals < threshold, np.isfinite(vals))

print("Removed", np.sum(~valid_mask), "exploded APL values")

# Convert related arrays BEFORE masking
vals = np.array(vals)
poses = np.array(poses)
...
restypes = restypes[valid_mask]

# Filter per-lipid lists
for n in range(len(lip_areas)):
    lip_areas[n] = np.array(lip_areas[n])
    mask = np.logical_and(lip_areas[n] < threshold,
                          np.isfinite(lip_areas[n]))
    lip_areas[n] = lip_areas[n][mask]    
rader = np.nanmean(vals)
rader_std = np.nanstd(vals)
